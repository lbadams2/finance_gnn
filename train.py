...
def get_neighbors(graph, symbol):
    pass
...
def train(x_windows, next_day_close_values_norm, next_day_close_values, y_normalizer, ti_norm, symbol, graph):
    test_split = 0.9 # the percent of data to be used for testing
    n = int(x_windows.shape[0] * test_split)
    neighbors = get_neighbors(graph, symbol)
    ...
    # can't use keras, train all stocks one time step at a time, only create one model
...
if __name__ == '__main__':
    ...
    for i in range(dim):
        if row_sums[i] + col_sums[i] > 0:
            ...
            train(x_windows, next_day_close_values_norm, next_day_close_values, y_normalizer, ti_norm, ticker, member_graph)
